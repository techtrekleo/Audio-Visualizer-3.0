...
interface OptimizedControlsProps {
    ...
    onUpdateBatchRow?: (index: number, field: 'audio' | 'subtitle' | 'cta' | 'cover', file: File) => void;
                                    ...
                                    <div className="flex flex-col gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                                        {props.batchQueue.map((item, index) => (
                                            <div key={item.id} className={`flex items-center gap-2 p-2 rounded-lg border ${index === props.currentBatchIndex ? 'bg-cyan-900/30 border-cyan-500/50' : 'bg-gray-800 border-gray-700'}`}>
                                                ...
                                                {/* Cover Image Upload */}
                                                <label className={`flex-1 flex items-center gap-2 px-3 py-2 rounded cursor-pointer transition-colors ${item.coverImageFile ? 'bg-pink-600/20 text-pink-300 border border-pink-600/30' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'}`}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 flex-shrink-0">
                                                        <path fillRule="evenodd" d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z" clipRule="evenodd" />
                                                    </svg>
                                                    <span className="text-xs truncate max-w-[120px]">{item.coverImageFile ? '已載入封面' : '選封面(選)'}</span>
                                                    <input
                                                        type="file"
                                                        className="hidden"
                                                        accept="image/*"
                                                        onChange={(e) => {
                                                            if (e.target.files?.[0]) {
                                                                props.onUpdateBatchRow?.(index, 'cover', e.target.files[0]);
                                                            }
                                                        }}
                                                    />
                                                </label>
                ...
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div className="space-y-2">
                            ...
                            {(props.resolution === Resolution.RATIO_9_16_1080 || props.resolution === Resolution.RATIO_9_16_720) && (
                                <div className="bg-purple-500/20 border border-purple-400/30 rounded-lg p-3 text-sm">
                                    <div className="flex items-center gap-2 text-purple-300">
                                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                                        </svg>
                                        <span className="font-medium">直式 9:16 格式</span>
                                    </div>
                                    <p className="text-purple-200 text-xs mt-1">
                                        適合 YouTube Shorts、TikTok、Instagram Reels 等直式影片平台
                                    </p>
                                </div>
                            )}
                        </div>
