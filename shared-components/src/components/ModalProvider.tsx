...
export const ModalProvider: React.FC<ModalProviderProps> = ({ children }) => {
  ...
  const openContactModal = () => {
    console.log('ModalProvider: openContactModal called');
    const modal = document.getElementById('contactModal');
    if (modal) {
      modal.style.display = 'block';
      document.body.style.overflow = 'hidden';
      console.log('ModalProvider: Contact modal displayed');
    } else {
      console.error('ModalProvider: contactModal element not found');
    }
  };

  const closeContactModal = () => {
    console.log('ModalProvider: closeContactModal called');
    const modal = document.getElementById('contactModal');
    if (modal) {
  ...
  const openPrivacyModal = () => {
    console.log('ModalProvider: openPrivacyModal called');
    const modal = document.getElementById('privacyModal');
    if (modal) {
      modal.style.display = 'block';
      document.body.style.overflow = 'hidden';
      console.log('ModalProvider: Privacy modal displayed');
    } else {
      console.error('ModalProvider: privacyModal element not found');
    }
  };

  const closePrivacyModal = () => {
    console.log('ModalProvider: closePrivacyModal called');
    const modal = document.getElementById('privacyModal');
    if (modal) {
      modal.style.display = 'none';
      document.body.style.overflow = 'auto';
  ...
  const openTermsModal = () => {
    console.log('ModalProvider: openTermsModal called');
    const modal = document.getElementById('termsModal');
    if (modal) {
      modal.style.display = 'block';
      document.body.style.overflow = 'hidden';
      console.log('ModalProvider: Terms modal displayed');
    } else {
      console.error('ModalProvider: termsModal element not found');
    }
  };

  const closeTermsModal = () => {
    console.log('ModalProvider: closeTermsModal called');
    const modal = document.getElementById('termsModal');
    if (modal) {
      modal.style.display = 'none';
      document.body.style.overflow = 'auto';
  ...
  return (
    <>
          ...
          <div className="contact-options">
            ...
            <a href="https://www.youtube.com/@%E9%9F%B3%E6%A8%82%E8%84%88%E5%8B%95SonicPulse" target="_blank" className="contact-option">
              ...
              <div className="contact-option-text">
                ...
                <div className="contact-option-desc">音樂脈動-Sonic Pulse</div>
              </div>
            </a>
          </div>
        </div>
      ...
      {/* 隱私政策彈出視窗 */}
      <div id="privacyModal" className="privacy-modal">
      ...
      {/* 使用條款彈出視窗 */}
      <div id="termsModal" className="terms-modal">
